...
Always `true` (feature inactive if not configured). Set it to true in the code and also in the application.yaml.
...
Place in the same package as the component using it

## 4. Delegate record creation and test setup

Delegate to `configuration-properties-record-pattern`.
